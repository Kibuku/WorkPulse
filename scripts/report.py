...
CLI:
    python -m scripts.report daily                  # today
    python -m scripts.report daily --date 2026-06-10
    python -m scripts.report weekly                 # week ending today
    python -m scripts.report weekly --ending 2026-06-15
    python -m scripts.report daily --no-llm
    python -m scripts.report daily --email          # send via SMTP
    python -m scripts.report daily --dry-run        # compute + render, no write
"""
...
from scripts import atoms, capture as capmod, consolidate, db, think
from scripts.common import ROOT, ensure_dir, load_config


_SKILLS = {
    "daily":  ROOT / "skills" / "report-daily.md",
    "weekly": ROOT / "skills" / "report-weekly.md",
}
...
def send_report_email(subject: str, body_md: str, cfg: dict | None = None) -> bool:
    ...
    if not (smtp_user and from_addr and to_addr):
        return False
    try:
        from scripts.wp_secrets import get as get_secret  # type: ignore

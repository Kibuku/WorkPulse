...
CLI:
    python -m scripts.coach status            # diagnostic dump
    python -m scripts.coach next              # the would-say message, or "(silent)"
    python -m scripts.coach gate              # just the gate state
"""
...
from scripts import consolidate, db
from scripts.common import ROOT, load_config


_SKILL_PATH = ROOT / "skills" / "coach.md"
...
def next_message(con: sqlite3.Connection, *,
    ...
    con.execute(
        ...
        """,
        (sr_id, datetime.now(timezone.utc).isoformat(),

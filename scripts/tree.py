...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="WorkPulse stream tree")
    sub = ap.add_subparsers(dest="cmd")
    sub.add_parser("show")
    sub.add_parser("validate")
    p1 = sub.add_parser("path"); p1.add_argument("key")
    p2 = sub.add_parser("children"); p2.add_argument("key")
    p3 = sub.add_parser("descendants"); p3.add_argument("key")
    args = ap.parse_args()
    ...
    if args.cmd == "show":
        ...
        print(" › ".join(path(args.key)))
    elif args.cmd == "children":
        print(json.dumps(children_of(args.key), indent=2))
    elif args.cmd == "descendants":
        print(json.dumps(descendants(args.key), indent=2))
    else:
        ap.print_help()
